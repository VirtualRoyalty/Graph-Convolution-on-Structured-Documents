...
class Adjacency(Layer):
    ...
    def call(self, inputs):  # NOTE: input_list contains a list of
        ...
        # flatten
        adj_0 = tf.reshape(adj_0, shape=(adj_0.shape[0]*adj_0.shape[1], 1))
        adj_1 = tf.reshape(adj_1, shape=(adj_1.shape[0]*adj_1.shape[1], 1))
        adj_2 = tf.reshape(adj_2, shape=(adj_2.shape[0]*adj_2.shape[1], 1))

        # forward pass
        adj_0 = tf.matmul(self.w0_1, adj_0 )
        adj_0 = tf.nn.relu(adj_0)
        adj_0 = tf.matmul(self.w0_2, adj_0 )
        adj_0 = tf.nn.relu(adj_0)

        adj_1 = tf.matmul(self.w1_1, adj_1 )
        adj_1 = tf.nn.relu(adj_1)
        adj_1 = tf.matmul(self.w1_2, adj_1 )
        adj_1 = tf.nn.relu(adj_1)

        adj_2 = tf.matmul(self.w2_1, adj_2 )
        adj_2 = tf.nn.relu(adj_2)
        adj_2 = tf.matmul(self.w2_2, adj_2  )
        adj_2 = tf.nn.relu(adj_2)
