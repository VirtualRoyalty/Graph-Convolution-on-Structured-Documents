...
class ObjectTree:
	...
	def connect(self, plot=False, export_df=False):
		...
		nearest_dest_ids_vert = [ x if x >= 0 else len(df)+x  for x in nearest_dest_ids_vert]
		...
		# ==================== horizontal =================================== #
		# create df for plotting lines
		nearest_dest_ids_hori = [ x if x >= 0 else len(df)+x  for x in nearest_dest_ids_hori]
